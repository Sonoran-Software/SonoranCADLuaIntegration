Config = {communityID = nil, apiKey = nil, apiUrl = nil, postTime = nil, serverId = nil, primaryIdentifier = nil, apiSendEnabled = nil, debugMode = nil, updateBranch = nil, enableCanary = false,
	latestVersion = '', apiVersion = -1, plugins = {}, proxyUrl = ''}
...
for k, v in pairs(json.decode(conf)) do
	...
	local val = nil
	if cvar ~= 'NONE' and cvar ~= 'statusLabels' and ((Config[k] == '' or Config[k] == nil) or Config[k] == 'serverId') then
...
AddEventHandler('SonoranCAD::core:sendClientConfig', function()
	local config = {communityID = Config.communityID, postTime = Config.postTime, serverId = Config.serverId, primaryIdentifier = Config.primaryIdentifier, apiSendEnabled = Config.apiSendEnabled,
		debugMode = Config.debugMode, devHiddenSwitch = Config.devHiddenSwitch, statusLabels = Config.statusLabels}
...
CreateThread(function()
	...
	performApiRequest({}, 'GET_SERVERS', function(response)
		...
		if ServerInfo == nil then
			needSetup = true
			serverObj = {id = serverId, name = 'Server ' .. serverId, description = 'Server ' .. serverId, signal = '', listenerPort = GetConvar('netPort', '0'), mapIp = '', differingOutbound = false,
				outboundIp = '', enableMap = true, mapType = 'NORMAL'}
		else
